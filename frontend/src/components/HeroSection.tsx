...
const HeroSection = ({ movies }: HeroSectionProps) => {
  ...
  return (
    <div className="relative w-full h-[55vh] md:h-[75vh] rounded-2xl overflow-hidden mb-8 group shadow-2xl border border-gray-800/50">
      ...
      <button 
        onClick={handlePrev}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-3 bg-black/30 hover:bg-red-600/80 text-white rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 hidden md:block"
      >
        <ChevronLeft size={28} />
      </button>
      ...
      <button 
        onClick={handleNext}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-3 bg-black/30 hover:bg-red-600/80 text-white rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 hidden md:block"
      >
        ...
        {movies.map((_, idx) => (
            <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                    idx === currentIndex ? 'bg-red-600 w-6' : 'bg-gray-500/50 w-2'
                }`}
            />
        ))}
      </div>
