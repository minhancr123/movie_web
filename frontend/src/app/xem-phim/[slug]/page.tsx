...
export default async function WatchPage({
    ...
    // Fetch Related Movies (with strict fallback)
    let relatedMovies = [];
    try {
        if (movie.category && movie.category.length > 0) {
            const relatedData = await getMoviesByCategory(movie.category[0].slug, 1);
            const list = relatedData?.data?.items || relatedData?.items || [];
            relatedMovies = list.filter((m: any) => m.slug !== movie.slug).slice(0, 12);
        }
    } catch (error) {
        console.error("Failed to load category movies", error);
    }

    // Fallback to Latest Movies if no related movies found
    if (relatedMovies.length === 0) {
        try {
            const latestData = await getLatestMovies(1);
            const list = latestData?.items || [];
            relatedMovies = list.filter((m: any) => m.slug !== movie.slug).slice(0, 12);
        } catch (error) {
            console.error("Failed to load latest movies fallback", error);
        }
    }
